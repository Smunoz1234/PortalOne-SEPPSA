...
//Generar certificado
$Param = array(
    "'" . FormatoFecha($_POST['FechaInicial']) . "'",
    "'" . FormatoFecha($_POST['FechaFinal']) . "'",
    "'" . $_SESSION['NITProv'] . "'",
    "'" . $_SESSION['CodigoSAPProv'] . "'",
);

$SQLCert = EjecutarSP("usp_GenerarCertificado", $Param);

if (!sqlsrv_has_rows($SQLCert)) {
    echo "No hay información para generar el certificado solicitado.";
    exit();
}
...
// Cabecera de página
    public function Header()
    {
        ...
        $this->SetFont('Arial', '', 8);
        $this->Cell(20, 5, utf8_decode($_POST['FechaInicial']), 0, 0, 'L');
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(30, 5, utf8_decode('Fecha final:'), 0, 0, 'L');
        $this->SetFont('Arial', '', 8);
        $this->Cell(20, 5, utf8_decode($_POST['FechaFinal']), 0, 0, 'L');
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(35, 5, utf8_decode('Fecha expedición:'), 0, 0, 'L');
        $this->SetFont('Arial', '', 8);
        $this->Cell(20, 5, date('Y-m-d h:i a'), 0, 0, 'L');
        $this->Ln();
...
$pdf->MultiCell(0, 5, utf8_decode('Las personas jurídicas podrán entregar los certificados de retención en la fuente en forma continua, impresa por computador, sin necesidad de firma autógrafa. Artículo 10 Decreto 0836 de 1991.'), 0, 'L');
$pdf->Ln(5);
